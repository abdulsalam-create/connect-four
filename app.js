...
for (let i = 0, len = squares.length; i < len; i++) {
  ...
  (function (index) {
    ...
    squares[i].onclick = function () {
      ...
      if(squares[index + 7].classList.contains('taken')) {
        ...
        //if the square below your current square is not taken, you cant't go here
      } else {
        text.innerHTML = 'Cant go here'
  ...
  function checkBoard() {
    ...
    //now take the values in each winningArrays and plug them into squares
    for (let y = 0; y < winningArrays.length; y++) {
      ...
      //now check those arrays to see wheater they all have player-one
      if (square1.classList.contains('player-one') &&
          square2.classList.contains('player-one') &&
          square3.classList.contains('player-one') &&
          square4.classList.contains('player-one')) {
        // if they do player-one as the winner
